...
class Element{
  constructor(dataset={}){this.dataset=dataset;this.listeners={};this.style={};this.className='';this.innerHTML='';this.hidden=false;this.value='';this.classList={toggle:(name,on)=>{this.className=on?name:''}}}
...
function makeDocument(){
  const elements=Object.fromEntries(['#score','#best','#lives','#timer','#state','#shield','#spread','#double','#skillName','#skillCharge','#skillMeter','#skillStatus','#skillButton','#notice','#rank','#overlay','#modal','#fighterOptions','#pause','#restart','#rankBtn'].map(selector=>[selector,new Element()]));
  const dynamic={};
...
test('renders HUD, dynamic fighter choices, and canvas fallback from a session snapshot',()=>{
  ...
  const page=Presentation.create({document:fake.document,canvas:fake.canvas,catalog:Catalog,onIntent:intent=>intents.push(intent)});
...
test('turns overlays and touch controls into intents while keeping ranking local',()=>{
  ...
  assert.deepEqual(intents.splice(0),[{type:'start'},{type:'direction',direction:'up',pressed:true},{type:'direction',direction:'up',pressed:false},{type:'skill'}]);
  fake.elements['#rankBtn'].fire('click');
  assert.match(fake.elements['#modal'].innerHTML,/排行榜/);
  fake.dynamic['#close'].fire('click');
  page.render(snapshot({game:{status:'over'},view:{overlay:'ended',canRegister:true,endReason:'death'}}));
