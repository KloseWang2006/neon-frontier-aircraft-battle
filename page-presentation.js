  ...
  function create({ document, canvas, catalog, onIntent }) {
    ...
    function guideMarkup() {
      const fighterCards = catalog
        ...
        .join('');
      return `<h2>游戏说明</h2><h3>操作与基础规则</h3><ul><li>电脑端按住方向键或 W/A/S/D；手机端在战场内拖动手指，战机会按距离自适应加速追随。两端均可在战场下方 60% 区域四方向移动；武器自动向上射击。</li><li>Q 是充能技：击毁普通、快速、精英敌机分别获得 2 / 3 / 5 能量；满 100 能量并结束 18 秒冷却后可释放。</li><li>E 是当前战机的独立普通技；Space、右侧按钮或手机技能键可暂停 / 继续。初始 3 点生命，敌弹或撞机造成 1 次伤害，受击后有 1 秒无敌保护且命中敌弹立即消失。</li><li>普通、快速、精英敌机随机出现；达到 300 分后敌机会开始发射敌弹。</li></ul><h3>普通敌军难度</h3><ul><li>0–1万分：每 900ms 生成，敌弹每 1100ms 发射，敌机速度为 85 / 135 / 62。</li><li>1–3万分保持标准档；3–5万分小幅提高；5万分后进入高压档。Boss 战期间暂停普通敌军，击败 Boss 后按当前分数恢复。</li></ul><h3>强化包</h3><ul><li>击毁敌机有合计 10% 概率掉落：护盾 2.5%、散射弹 3%、双倍火力 3.5%、治疗药水 1%。</li><li>护盾抵挡一次命中；散射弹每次发射三发；双倍火力将间隔从 180ms 缩短到 90ms；前三者持续 8 秒。</li><li>治疗药水立即恢复 1 格生命，满血时仍会消耗但没有效果。</li></ul><h3>Boss、通关与排行</h3><ul><li>首次达到 10,000、30,000、50,000、100,000 分时依次触发 Boss 战，并清空普通敌军。Boss 横移并交替使用扇形与三连弹幕。</li><li>每次击败 Boss 奖励 2,000 分；击败第一 Boss 获得上榜资格，击败最终 Boss 即通关。</li><li>结束时，获得资格的玩家可登记 ID。成绩只保存在当前浏览器，按分数降序、同分按用时升序保留前 10 条；暂停不计时。</li></ul><h3>战机图鉴</h3>${fighterCards}<button class="btn modal-secondary" id="closeGuide">关闭说明</button>`;
    }
