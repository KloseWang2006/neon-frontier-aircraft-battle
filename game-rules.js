(function(root,factory){const catalog=typeof module==='object'&&module.exports?require('./fighter-catalog.js'):root.FighterCatalog;const rules=factory(catalog);if(typeof module==='object'&&module.exports)module.exports=rules;root.GameRules=rules})(globalThis,FighterCatalog=>{
  ...
  function enemy(kind,id,x){
    const q=kind==='elite'?{w:58,h:52,hp:3,score:350,speed:68}:kind==='fast'?{w:30,h:30,hp:1,score:150,speed:155}:{w:38,h:38,hp:1,score:100,speed:95};
    return{kind,id,x,y:-q.h,...q,phase:0};
  }

  function spawn(s,random){
    const roll=random(),kind=roll>.82?'elite':roll>.55?'fast':'normal',w=kind==='elite'?58:kind==='fast'?30:38;
  ...
  function fighter(s){return FighterCatalog.get(s.fighterId)||FIGHTERS.azure}
  ...
  function step(state,{dt=16,input={},random=Math.random}={}){
    ...
    const s=clone(state),events=[],ms=dt;
    s._dt=ms;
    s.elapsedMs+=ms;
    ...
    if(!s.boss){
      ...
      if(trigger){
        ...
        s.triggered.push(trigger);s.enemies=[];s.enemyBullets=[];
        s.boss={stage:i+1,trigger,x:160,y:65,w:160,h:120,hp:BOSS_HP[i],maxHp:BOSS_HP[i],attackClock:0,pattern:0,phase:0};
        events.push({type:'boss-started',stage:i+1});
      }else{
        s.spawnClock+=ms;
        if(s.spawnClock>=700){s.spawnClock=0;spawn(s,random)}
        s.enemies=s.enemies.map(e=>({...e,y:e.y+e.speed*ms/1000,x:e.kind==='fast'?e.x+Math.sin((e.phase+=ms/300))*42*ms/1000:e.x}));
        s.enemyFireClock+=ms;
        if(s.score>=300&&s.enemies.length&&s.enemyFireClock>=900){
          s.enemyFireClock=0;
          const e=s.enemies[Math.floor(random()*s.enemies.length)],cx=e.x+e.w/2;
          s.enemyBullets.push({x:cx-4,y:e.y+e.h,w:8,h:14,vx:(s.player.x+30-cx)*.28,vy:235});
        }
      }
    }else{
      const b=s.boss;
      b.phase+=ms;b.x=160+Math.sin(b.phase/900)*75;b.attackClock+=ms;
      if(b.attackClock>=1050){
        b.attackClock=0;b.pattern=1-b.pattern;
        const n=b.pattern?6:3;
        for(let i=0;i<n;i++)s.enemyBullets.push({x:b.x+76,y:b.y+110,w:8,h:14,vx:(i-(n-1)/2)*55,vy:230+b.stage*12});
      }
    }

    const used=new Set(),dead=new Set();
    for(let bi=0;bi<s.bullets.length;bi++){
      ...
      if(s.boss&&hit(b,s.boss)){
        ...
        if((s.boss.hp-=(b.damage??1))<=0){
          ...
          if(boss.stage===1){s.rankEligible=true;events.push({type:'ranking-unlocked'})}
          if(boss.stage===4){s.status='success';events.push({type:'run-ended',reason:'victory'})}
        }
        continue;
      }
    ...
    if(input.skill&&s.skillCharge>=SKILL_MAX&&s.skillCooldownMs===0){
      ...
      fighter(s).rules.activate({state:s,events,inRange:target=>inShockwaveRange(s,target)});
    }
    ...
    s.powerups=s.powerups.map(p=>({...p,y:p.y+p.vy*ms/1000})).filter(p=>p.y<H+30);
    for(const p of s.powerups)if(hit(p,s.player)){
      if(p.kind==='shield'){s.shieldMs=8000;s.shieldAvailable=true}
      else if(p.kind==='spread')s.spreadMs=8000;
      else if(p.kind==='double')s.doubleMs=8000;
      else if(s.player.lives<MAX_LIVES){s.player.lives++;s.healFlashMs=700;events.push({type:'life-restored'})}
      p.taken=true;
    }
